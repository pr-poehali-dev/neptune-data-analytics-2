...
type Order = { id: number; title: string; description: string; status: string; file_url: string | null; created_at: string; client_name: string; client_email: string }
...
export default function SupportStaffPage() {
  ...
  const loadOrders = () => {
    api.orders.list().then(data => setOrders(Array.isArray(data) ? data : []))
  }
  ...
  const handleSetFile = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!selectedOrder || !fileUrl.trim()) return
    const res = await api.orders.setFile(selectedOrder.id, fileUrl)
    if (res.success) {
  ...
  const filteredOrders = orderStatusFilter ? orders.filter(o => o.status === orderStatusFilter) : orders
      ...
      <div className="container mx-auto px-4 py-8 max-w-6xl">
        ...
        <div className="flex gap-2 mb-6">
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium transition-colors ${tab === 'tickets' ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground hover:text-foreground'}`}
          >
            <Icon name="MessageCircle" size={15} />
            Обращения
            {openCount > 0 && <span className="bg-destructive text-destructive-foreground rounded-full w-5 h-5 flex items-center justify-center text-xs">{openCount}</span>}
          </button>
          <button
            onClick={() => setTab('orders')}
            className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium transition-colors ${tab === 'orders' ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground hover:text-foreground'}`}
          >
                    ...
                    <div ref={chatRef} className="flex-1 overflow-y-auto p-4 space-y-3">
                      ...
                      {ticketReplies.map(reply => (
                        <div key={reply.id} className={`flex ${reply.is_admin ? 'justify-end' : 'justify-start'}`}>
                          <div className={`max-w-[80%] rounded-2xl px-4 py-3 text-sm ${reply.is_admin ? 'bg-primary text-primary-foreground' : 'bg-muted'}`}>
                            <p>{reply.text}</p>
        ...
        {tab === 'orders' && (
          <>
            <div className="mb-6 flex items-center justify-between flex-wrap gap-3">
              <div>
                <h1 className="text-3xl font-bold">Заказы клиентов</h1>
                <p className="text-muted-foreground mt-1">{orders.length} заказов всего</p>
              </div>
              <div className="flex gap-2 flex-wrap">
                {[{ value: '', label: 'Все' }, { value: 'new', label: 'Новые' }, { value: 'in_progress', label: 'В работе' }, { value: 'review', label: 'На проверке' }, { value: 'done', label: 'Готово' }].map(f => (
                  <button
                    key={f.value}
                    onClick={() => setOrderStatusFilter(f.value)}
                    className={`px-3 py-1.5 rounded-full text-sm font-medium transition-colors ${orderStatusFilter === f.value ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground hover:text-foreground'}`}
                  >
                    {f.label}
                  </button>
                ))}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="space-y-3">
                {filteredOrders.length === 0 && (
                  <Card className="border-dashed">
                    <CardContent className="py-12 text-center text-muted-foreground">
                      <Icon name="ShoppingBag" className="h-12 w-12 mx-auto mb-4 opacity-30" />
                      <p>Заказов нет</p>
                    </CardContent>
                  </Card>
                )}
                {filteredOrders.map(order => {
                  const s = ORDER_STATUS[order.status] || { label: order.status, variant: 'secondary' as const }
                  return (
                    <Card
                      key={order.id}
                    ...
                    >
                      <CardContent className="p-4">
                        <div className="flex items-start justify-between gap-2 mb-2">
                          <div className="flex-1 min-w-0">
                            <p className="font-semibold truncate text-sm">#{order.id} {order.title}</p>
              ...
              <div>
                {selectedOrder ? (
                  <Card className="flex flex-col h-[580px] overflow-y-auto">
                    <CardHeader className="pb-3 border-b">
                      <CardTitle className="text-sm font-semibold truncate">
                        #{selectedOrder.id} {selectedOrder.title}
                      </CardTitle>
                      <p className="text-xs text-muted-foreground">{selectedOrder.client_name} · {selectedOrder.client_email}</p>
                    </CardHeader>
                    <CardContent className="p-4 space-y-4">
                      {selectedOrder.description && (
                        <div>
                          <p className="text-xs text-muted-foreground mb-1 font-medium">Описание</p>
                          <p className="text-sm bg-muted rounded-lg p-3">{selectedOrder.description}</p>
                        </div>
                      )}

                      <div>
                        <p className="text-xs text-muted-foreground mb-2 font-medium">Статус заказа</p>
                        <Select value={selectedOrder.status} onValueChange={v => handleOrderStatus(selectedOrder.id, v)}>
                      ...
                      {selectedOrder.file_url && (
                        <div>
                          <p className="text-xs text-muted-foreground mb-1 font-medium">Прикреплённый файл</p>
                          <a href={selectedOrder.file_url} target="_blank" rel="noreferrer" className="text-sm text-primary underline break-all">{selectedOrder.file_url}</a>
                        </div>
                      )}

                      <form onSubmit={handleSetFile} className="space-y-2">
                        <p className="text-xs text-muted-foreground font-medium">Прикрепить файл (ссылка)</p>
                        <div className="flex gap-2">
                          <Input
                            value={fileUrl}
                          ...
                          />
                          <Button type="submit" size="sm" className="shrink-0">Сохранить</Button>
                        </div>
                      </form>
                    </CardContent>
                  </Card>
                  ...
                  <Card className="h-[580px] flex items-center justify-center border-dashed">
                    <div className="text-center text-muted-foreground">
                      <Icon name="ShoppingBag" className="h-12 w-12 mx-auto mb-3 opacity-30" />
                      <p className="text-sm">Выберите заказ</p>
                    </div>
                  </Card>
                )}
              </div>
